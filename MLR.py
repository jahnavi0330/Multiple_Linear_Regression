...
class TRAIN:
    ...
    def train_perfom(self):
        try:
            ...
            print(f'Loss by root_mean_squared_error : {root_mean_squared_error(self.y_train,self.y_train_pred)}')
        except Exception as e:
            error_type, error_msg, err_line = sys.exc_info()
            print(f'Error from Line {err_line.tb_lineno} -> type {error_type} -> {error_msg}')
    ...
    def test_data(self):
        try:
            ...
            print(f'Loss by root_mean_squared_error : {root_mean_squared_error(self.y_test, self.y_test_pred)}')
        except Exception as e:
            error_type, error_msg, err_line = sys.exc_info()
            print(f'Error from Line {err_line.tb_lineno} -> type {error_type} -> {error_msg}')
